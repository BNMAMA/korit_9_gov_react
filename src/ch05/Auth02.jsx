...
function Auth02() {
    ...
    const signinRequest = async() => {
        try {
            ...
            const {accessToken} = response.data; //application access 토큰 확인!
            localStorage.setItem("AccessToken", accessToken);
            await getPincipalRequest();
        } catch (error) {
    ...
    const getPincipalRequest = async() => {
        try {
    ...
    return <>
        ...
        <div>
            <input type="text" placeholder="사용자 이름" name="username" value={inputValue.username} onChange={handleInputOnChange}/>
            <span>{inputMessage.usernmae}</span>
        </div>
        <div>
        ...
        <button disabled={signinButtonDisabled} onClick={handleSingninOnClick}>로그인</button>
    </>
}
